            ...
            if maxSimilar > threshold:
                for ii in range(0, 256):
                    centrodR[clusterGroupId][ii] = centrodR[clusterGroupId][ii] * clusterCount[clusterGroupId] \
                                                   / (clusterCount[clusterGroupId] + 1) + temphistR[ii] * 1.0 / (
                                                               clusterCount[clusterGroupId] + 1)
                    centrodG[clusterGroupId][ii] = centrodG[clusterGroupId][ii] * clusterCount[clusterGroupId] \
                                                   / (clusterCount[clusterGroupId] + 1) + temphistG[ii] * 1.0 / (
                                                               clusterCount[clusterGroupId] + 1)
                    centrodB[clusterGroupId][ii] = centrodB[clusterGroupId][ii] * clusterCount[clusterGroupId] \
                                                   / (clusterCount[clusterGroupId] + 1) + temphistB[ii] * 1.0 / (
                                                               clusterCount[clusterGroupId] + 1)
...
if __name__ == '__main__':
    ...
    # 进行Kmeans聚类
    kms = KMeans(n_clusters=cluster_size)
    # 进行聚合，实现的是每一帧图像的数据属于哪一类
    y = kms.fit_predict(histogram)

    # 聚合结果图像信息
    frame_label = {}
    for i in range(len(kms.cluster_centers_)):
        frame_label[i] = kms.cluster_centers_[i]

    # 对每一类，找到视频帧图像中与之最接近的图像
    hist0 = histogram[0]
    key_id = []
    for i in range(len(frame_label)):
        keyframe = frame_label[i]
